...
fn parse_maze_file<P: AsRef<Path>>(path: P) -> io::Result<(Vec<Vec<char>>, usize, usize)> {
    ...
    let width: usize = lines.next()
        .ok_or_else(|| io::Error::new(io::ErrorKind::InvalidData, "Отсутствует информация о ширине"))?
        .map_err(|e| io::Error::new(io::ErrorKind::InvalidData, e))?
        .parse()
        .map_err(|_| io::Error::new(io::ErrorKind::InvalidData, "Некорректная ширина"))?;
    
    let height: usize = lines.next()
        .ok_or_else(|| io::Error::new(io::ErrorKind::InvalidData, "Отсутствует информация о высоте"))?
        .map_err(|e| io::Error::new(io::ErrorKind::InvalidData, e))?
        .parse()
    ...
    if maze.len() != height || maze.iter().any(|row| row.len() != width) {
        return Err(io::Error::new(io::ErrorKind::InvalidData, "Размеры лабиринта не соответствуют указанным"));
    }
...
fn get_adjacent_cells(point: &Point, maze: &Vec<Vec<char>>, width: usize, height: usize) -> Vec<Point> {
